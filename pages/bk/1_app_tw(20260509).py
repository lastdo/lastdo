...
def generate_ai_insights(
    ...
    # ── 計算真實本益比（年EPS）──
    pe_section = ""
    if eps_y is not None and not eps_y.empty:
        latest_annual_eps = float(eps_y.sort_values("date").iloc[-1]["eps"])
        if latest_annual_eps > 0:
            pe_ratio = end_price / latest_annual_eps
            pe_section = f"- **【提供數據】年EPS（最新年度）**：NT${latest_annual_eps:.2f} 元\n- **【提供數據】本益比（股價/年EPS）**：{end_price:.2f} / {latest_annual_eps:.2f} = **{pe_ratio:.1f} 倍**\n"
        else:
...
### 4. 基期風險評估
必須包含：
- **本益比河流圖位置**：【提供數據】年EPS已提供於上方。請計算目前PE倍數，並與近三年（2023-2025）的實際PE區間比較，說明目前PE處於近三年的低/中/高檔（禁止使用更久遠的歷史平均，避免失真）。若能得知此股近三年PE區間，請引用；否則根據提供的股價與EPS數據自行推算可能合理範圍。
- **股價所在位階**：根據上方提供的股價 JSON 數據，計算近60筆交易日的最高/最低價，說明目前收盤價在此區間的幾%分位，並補充說明近一年與近兩年大約在幾%分位。
- **殖利率安全邊際**：【提供數據】年EPS已提供於上方。請根據此公司近三年的實際現金股利配發記錄（需引用具體年度股利金額或配發率），估算本年度預估現金股利，計算目前股價對應的殖利率（股利÷股價×100%），並與該股近三年各年度的殖利率數字比較，判斷目前殖利率處於近三年的高／中／低檔，說明是否提供足夠的股息保護墊（通常 > 3% 視為具保護性）。若公司不配息或配發率極低，請直接說明並解釋原因（如成長型公司傾向保留盈餘）。禁止在無任何依據的情況下捏造配發率或股利數字。
