...
def render_watchlist_adder(result_df: pd.DataFrame, family_id: str, finmind_token: str = "") -> None:
    def _label(row: pd.Series) -> str:
        return (
            ...
            f"外資近N日買超 {format_watchlist_number(row['foreign_net_buy_lot'])} 張 | "
    ...
    def _caption(selected: dict, chart_df: pd.DataFrame) -> str:
        ...
        return (
            ...
            f"季線溢價 {format_watchlist_number(premium, '%')}"
        )

    render_watchlist_adder_base(
        result_df,
        family_id,
        select_columns=["stock_id", "stock_name", "market", "close", "foreign_net_buy_lot", "pe_ratio"],
        numeric_columns=["close", "foreign_net_buy_lot", "pe_ratio"],
        label_builder=_label,
        chart_loader=get_finmind_price_chart_data,
        selectbox_key="chip_watchlist_symbol",
        add_button_key="chip_watchlist_add",
