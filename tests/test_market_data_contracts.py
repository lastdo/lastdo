...
class MarketDataContractTests(unittest.TestCase):
    ...
    def test_fetch_market_revenue_follows_mops_redirect_manually(self):
        class FakeClient:
            def __init__(self, *args, **kwargs):
                pass
            ...
            def get(self, url, headers=None):
                requested_urls.append(url)
                if url.endswith("redirected.html"):
                    return FakeResponse(200, url)
                if url.endswith("t21sc03_115_5_0.html"):
                    return FakeResponse(307, url, {"location": "redirected.html"})
                return FakeResponse(200, url)
        ...
        class FakeFetcher:
            ...
            def _get_revenue_url(self, year, month, market, company_type):
                return "https://mopsov.twse.com.tw/nas/t21/sii/t21sc03_115_5_0.html"
        ...
        with patch("data_layer.mops_revenue.httpx.Client", FakeClient):
            with patch("data_layer.mops_revenue._read_mops_revenue_sections", return_value=[pd.DataFrame()]):
                with patch("data_layer.mops_revenue._parse_mops_revenue_tables", return_value=parsed):
                    result = mops_revenue._fetch_market_revenue_with_redirects(FakeFetcher(), 115, 5, "sii", 0)

        self.assertEqual(result["stock_id"].tolist(), ["2330"])
        self.assertIn("https://mopsov.twse.com.tw/nas/t21/sii/redirected.html", requested_urls)

    def test_read_mops_revenue_sections_rejects_security_page(self):
        with self.assertRaisesRegex(RuntimeError, "security page"):
            mops_revenue._read_mops_revenue_sections("FOR SECURITY REASONS, THIS PAGE CAN NOT BE ACCESSED")
    ...
    def test_fetch_mops_recent_revenue_skips_partial_latest_month(self):
        def fake_month_frame(year, month):
            ym = f"{year}{month:02d}"
            rows = 2 if ym == "11506" else 8
            return pd.DataFrame(
                {
                    "stock_id": [f"{i:04d}" for i in range(rows)],
                    "rev_ym": [ym] * rows,
                    "rev_yoy": [20.0] * rows,
                    "rev_cur": [300000] * rows,
                    "rev_ly": [250000] * rows,
        ...
        self.assertEqual(df["rev_ym"].drop_duplicates().tolist(), ["11506", "11505", "11504"])
        self.assertEqual(df.attrs["resolved_latest_ym"], "11506")
        self.assertEqual(df.attrs["selected_rev_months"], ("11506", "11505", "11504"))
        self.assertEqual(df.attrs["complete_rev_months"], ("11505", "11504"))
        self.assertEqual(df.attrs["skipped_incomplete_rev_months"], ("11506",))

    def test_fetch_mops_recent_revenue_uses_last_two_months_when_last_month_is_published(self):
        def fake_month_frame(year, month):
            ym = f"{year}{month:02d}"
            return pd.DataFrame(
                {
